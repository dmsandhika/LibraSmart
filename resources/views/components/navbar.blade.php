                ...
                <div class="flex items-center gap-4">
                    @if (Auth::user())
                        <div class="relative" data-twe-dropdown-ref>
                            <a class="rounded-md text-teal-600 border-teal-600 border-2 px-5 py-2.5 hover:bg-teal-600 hover:text-white "
                                type="button" id="dropdownMenuButton2"
                                data-twe-dropdown-toggle-ref>{{ Auth::user()->name }}</a>
                            <ul class="absolute z-[1000] float-left m-0 hidden min-w-max list-none overflow-hidden rounded-lg border-none bg-white bg-clip-padding text-base shadow-lg data-[twe-dropdown-show]:block k"
                                aria-labelledby="dropdownMenuButton2" data-twe-dropdown-menu-ref>

                                @if (Auth::user()->hasRole('admin'))
                                    <li>
                                        <a href="/dashboard"
                                            class="text-center block w-full whitespace-nowrap bg-white px-4 py-2 text-sm font-normal text-neutral-700 hover:bg-zinc-200/60 focus:bg-zinc-200/60 focus:outline-none active:bg-zinc-200/60 active:no-underline">Dashboard</a>
                                    </li>
                                @else
                                    <li>
                                        <a href=""
                                            class="text-center block w-full whitespace-nowrap bg-white px-4 py-2 text-sm font-normal text-neutral-700 hover:bg-zinc-200/60 focus:bg-zinc-200/60 focus:outline-none active:bg-zinc-200/60 active:no-underline"">Profil</a>
                                    </li>
                                    <li>
                                        <a href=""
                                            class="text-center block w-full whitespace-nowrap bg-white px-4 py-2 text-sm font-normal text-neutral-700 hover:bg-zinc-200/60 focus:bg-zinc-200/60 focus:outline-none active:bg-zinc-200/60 active:no-underline">Daftar
                                            Peminjaman</a>
                                    </li>
                                @endif
                                <li>
                                    <form action="{{ route('logout') }}" method="POST">
                                        ...
                                        <button
                                            ...
                                            href={{ route('logout') }} data-twe-dropdown-item-ref>Logout</button>
                                    </form>
                                </li>
                            </ul>
                        </div>
        ...
        <div id="mobile-menu" class="hidden md:hidden h-auto">
            <nav aria-label="Global" class="mt-4">
                <ul class="flex flex-col gap-4 text-sm w-full">
                    <li>
                        <a class="text-gray-500 transition hover:text-teal-600 hover:bg-gray-300 rounded-md w-full text-center px-5 py-2.5"
                            href="#">About</a>
                    </li>
                    <li>
                        <a class="text-gray-500 transition hover:text-teal-600 hover:bg-gray-300 rounded-md w-full text-center px-5 py-2.5"
